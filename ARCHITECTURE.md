...
`findTechniciansByName()` normalizes and searches across all available name fields (`name`, `displayName`, `fullName`, `firstName + lastName`, `nickname`). The search is case-insensitive substring matching, so `"gonzalo"` matches `"Gonzalo Ruiz"`.
...
```
intel_lookup(entity="technician", name="Gonzalo") → [{ id: 1234, name: "Gonzalo Ruiz" }]
```
