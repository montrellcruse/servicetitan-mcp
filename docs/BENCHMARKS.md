...
The measured v3.0.1 JavaScript build SHA-256 is `b9029359130a290ad67fbf3d6a7481f1cb3d644cfb089357a06b994c9605e629`, computed over sorted JavaScript paths plus NUL-separated contents. Only the aggregate results are retained here. Broader client-load, analytics-cache and retained-memory measurements below were not repeated for this description-only patch.
...
On September 5, 2026 at `d4801c6`, the client-load and analytics harnesses were repeated sequentially on Node 22.23.2 and 24.20.0 after correcting completed-mutation delivery semantics, query filters, and official-operation lookup precedence. The measured package identity is `a3f20d142ac0dbbfdb17367e9679e03ca285a7f1a08291de9c822c540d218701`; the harnesses bundled the corresponding client/executor source. The full protocol, soak, and retained-memory comparison above remains evidence for `786f15b` and was not repeated for these error-handling changes.
...
The later audit-sink isolation fix was validated by synchronous/asynchronous failure regressions and built-package SDK checks. These benchmarks were not repeated for that fix and do not measure custom audit-sink latency or delivery.
