...
- **Tool annotations** — every tool advertises MCP `readOnlyHint` / `destructiveHint` / `idempotentHint` hints derived from its operation, so hosts can badge read-only tools and gate destructive ones
