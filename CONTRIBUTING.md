...
- MCP tool annotations are derived from `operation`; only set `annotations` on a definition to override a specific hint (e.g. `idempotentHint: true` on a write that is safe to repeat)
