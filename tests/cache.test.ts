...
describe("ReferenceDataCache", () => {
  ...
  it("returns technician name from cache by ID", async () => {
    const loader = vi.fn().mockResolvedValue({
      data: [
        { id: 10, name: "Gonzalo R" },
        { id: 11, firstName: "Raheem", lastName: "K" },
      ],
      hasMore: false,
      page: 1,
    });
    ...
    expect(await cache.getTechnicianName(client, 10)).toBe("Gonzalo R");
    expect(await cache.getTechnicianName(client, 11)).toBe("Raheem K");
