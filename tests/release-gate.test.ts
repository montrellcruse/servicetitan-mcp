...
function fixture() {
  ...
  for (const dir of ["src", "tests", "scripts", "benchmarks", ".github/workflows", "docs/releases", "docs/contracts", "docs/evaluation"]) mkdirSync(join(cwd, dir), { recursive: true });
  writeFileSync(join(cwd, "glama.json"), '{"maintainers":["fixture"]}');
  writeFileSync(join(cwd, "docs/evaluation/README.md"), "Evaluation evidence\n");
...
describe("release acceptance enforcement", () => {
  ...
  it("invalidates evidence when credential exclusion rules or support guidance change", () => {
    const f = fixture();
    for (const file of [".dockerignore", ".gitignore", "SECURITY.md", "CONTRIBUTING.md", "ARCHITECTURE.md", "docs/contracts/README.md", "docs/releases/VALIDATION-v3.md", "docs/BENCHMARKS.md", "benchmarks/protocol.mjs", "glama.json", "docs/evaluation/README.md"]) {
